            ...
            body:
                ...
                Query(
                    ...
                    builder: (
                      ...
                      print("job board result ${result2}");
                      refetchlist = refetch;
                      ...
                      if (result2.data != null &&
                        ...
                        // jobBordList.clear();
                        for (Map item in result2.data!['getJobBoard']['data']) {
                          JobBoard temp =
                              JobBoard.fromJson(item as Map<String, dynamic>);
                          if (!jobBordList.contains(temp)) {
                            jobBordList.add(temp);
                          }
                        }
                        print(result2.data?['getJobBoard']['pageInfo']);
                        if (result2.data?['getJobBoard']['pageInfo'] != null) {
                          // pageNo = result2.data!['getJobBoard']['pageInfo']
                          //     ['currentPage'];
                          hasNextPage = result2.data!['getJobBoard']['pageInfo']
                              ['hasNextPage'];
                        }

                        print("job board length ${jobBordList.length}");
                        fbState.setJobBoardList(jobBordList);
                        // List<JobBoard> tempJobsList = [];
                        // //Filter out expired jobs----------
                        // for (JobBoard job in fbState.jobBoardList) {
                        //   // if job is after today or job is today but within 3 hrs of current time
                        //   if (job.jobDate!.isAfter(DateTime.now()) ||
                        //       (job.jobDate!.difference(DateTime.now()).inDays ==
                        //               0 &&
                        //           job.jobDate!
                      ...
                      return SmartRefresher(
                        ...
                        child: Container(
                          ...
                          color: zimkeyWhite,
                          child: jobBordList.isEmpty
                              ? emptyBookingWidget()
                              : pageNo == 1 && result2.isLoading
                                  ? Center(
                                  ...
                                  : ListView.builder(
                                      ...
                                      itemCount: jobBordList.length + 1,
                                      itemBuilder: (context, index) {
                                        return index >= jobBordList.length
                                            ? hasNextPage
                                                ? Center(
                                                    child:
...
class JobItemWidgetNew extends StatefulWidget {
  ...
  const JobItemWidgetNew({super.key, this.job, this.callAssignMutation});
                      ...
                      if (widget.job?.jobService?.isTeamService == true) {
                        showModalBottomSheet(
                            ...
                            builder: (context) => GetTeamsWidget(
                                  ...
                                  onTap: widget.callAssignMutation!,
                                ));
                      } else {
                        widget.callAssignMutation!(null);
                      }
                      //
                    },
                    child: Container(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${DateTime.parse('${widget.job!.jobDate}').day}-${DateTime.parse('${widget.job!.jobDate}').month}-${DateTime.parse('${widget.job!.jobDate}').year}  $hr:$min - $endHr:$endMin',
                  style: TextStyle(
                    color: zimkeyDarkGrey,
                    fontSize: 12,
                  ),
