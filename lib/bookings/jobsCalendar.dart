...
class _BookingsState extends State<JobsCalendar>
  ...
  List<PartnerCalendarItem> openJobs = [];
  List<PartnerCalendarItem> completedJobs = [];
  List<PartnerCalendarItem> progressJobs = [];
...
// }
    for (PartnerCalendarItem item in jobsCalendar) {
      // print('${item.partnerCalendarStatus.toString()}');
      String itemStatus;
      itemStatus = item.bookingServiceItem!.bookingServiceItemStatus.toString();
      itemStatus = itemStatus.split('.')[1];
      itemStatus = itemStatus.toLowerCase();
      switch (itemStatus) {
        case 'open':
          {
            if (item.partnerCalendarStatus.toString().contains('CANCELED'))
  ...
  Widget bookingWidget(
      String status, List<PartnerCalendarItem> joblist, Refetch? refetch) {
    return Container(
      child: ListView(
        children: [
          for (PartnerCalendarItem item in joblist) jobListItem(item, refetch),
        ],
      ),
    );
  }

  Widget jobListItem(PartnerCalendarItem jobitem, Refetch? refetch) {
    String endHr;
    String endMin;
...
//get areaname
    String? thisArea;
    if (jobitem.booking!.bookingAddress != null &&
        jobitem.booking!.bookingAddress!.areaId != null)
      for (Area area in fbState.areaList) {
        if (area.id == jobitem.booking!.bookingAddress!.areaId) {
          thisArea = area.name;
        }
      }
    /////
  ...
  Widget bookingServiceItem(b.BookingServices bookingServ, b.Bookings booking) {
    ...
    String? servIcon;
    if (bookingServ.service != null && bookingServ.service!.icon != null) {
      servIcon = bookingServ.service!.icon != null &&
              bookingServ.service!.icon!.url != null
          ? serviceImg + bookingServ.service!.icon!.url!
          : "";
    }
    bool isPng = false;
    ...
    return Query(
      ...
      builder: (QueryResult result,
        ...
        if (result.data != null &&
          ...
          for (Map item in result.data!['getPartnerCalendarItems']['data']) {
            PartnerCalendarItem temp;
            temp = PartnerCalendarItem.fromJson(item as Map<String, dynamic>);
        ...
        return NotificationListener<ScrollNotification>(
                ...
                fetchMore!(FetchMoreOptions(
                  ...
                  updateQuery: (previousResultData, fetchMoreResultData) {
                    ...
                    final List<dynamic> newData =
                        fetchMoreResultData!['getPartnerCalendarItems']['data'];
                    return {
          ...
          child: pageNo == 1 && result.isLoading
              ? Center(
