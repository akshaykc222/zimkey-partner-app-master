  ...
  filterOpenJobs() {
    ...
    for (PartnerCalendarItem item in jobsCalendar) {
      if (!item.bookingServiceItem!.bookingServiceItemStatus
              .toString()
              .contains("CLOSED") ||
          !item.partnerCalendarStatus.toString().contains('CANCELED')) {
        tempJobs.add(item);
      }
    }
    ...
    for (PartnerCalendarItem jobitem in jobsCalendar) {
      ...
      switch (jobStatus) {
        ...
        case 'CLOSED':
          {
            completedJobs++;
            if (jobitem != null && jobitem.booking != null)
              // jobitem.booking != null)
              // totalEarned = totalEarned + jobitem.booking.netPrice;
              break;
          }
      }
    }
